...
import { customLocale } from '@/common/constants';
...
import TutorialCarouselDots from './carousel-dots.component';
...
const TutorialBoundary: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  useEffect(() => {
    if (isFirstLogin && !isTutorialShown) {
      handleOnboarding();
    }
  }, [isFirstLogin, isTutorialShown]);

  const handleJoyrideCallback = (data: { index: any; status: any; type: any }) => {
    const { index, status, type } = data;

    if (status === 'finished' || status === 'skipped') {
      // Reset active step when Joyride finishes or gets skipped
      setCurrentStep(0);
      storageService.setStorage('TUTORIAL_SHOWN_OR_SKIPPED', JSON.stringify(true));
      refetchUser();
      return;
