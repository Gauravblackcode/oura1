...
const RolesPermissionsComponent: React.FC<RolesPermissionsProps> = ({
  ...
  const { enableAPIExpose, showYieldManagementPermission, showLookerTab } = useConfigs();
  ...
  const { permission: userPermission, isAdvertiser, isLoadingPermission = true } = useUser(); // using as userPermission to avoid readability conflict
          ...
          c4: {
            ...
            content: (
              <CustomPermission
                ...
                onPermissionSelect={onCustomPermissionSelect}
          ...
          c4: {
            ...
            content: (
              <CustomPermission
                disabled={disabled}
                options={
                  userPermission?.CREATIVE_TEMPLATE.fullAccess &&
  ...
  if (showLookerTab.isCugRolloutAdvertisers && userPermission?.INSIGHT_DASHBOARD.noAccess && isAdvertiser) {
    onlyReadable ? readableMeta[USER_TYPE.ADVERTISER].rows.splice(4, 1) : meta[USER_TYPE.ADVERTISER].rows.splice(4, 1);
  }
  ...
  const onRoleChange = (userRole: USER_ROLE) => {
    ...
    Object.keys(userType === USER_TYPE.ADVERTISER ? advertiser_permissions : publisher_permissions).forEach(key => {
      ...
      // if (userRole === USER_ROLE.SUPER_USER) {
      //   if (key === PermissionType.Wallet) {
      //     per[key] = PERMISSION_LEVELS.MANAGE_WALLET;
      //   } else if (key === PermissionType.ApprovalRequests) {
      //     per[key] = PERMISSION_LEVELS.ALL_REQUESTS;
      //   } else if (key === PermissionType.PublicApiAccess) {
      //     per[key] = PERMISSION_LEVELS.NO_ACCESS;
      //   } else if (key !== PermissionType.InsightDashboard) {
      //     per[key] = PERMISSION_LEVELS.VIEW_ONLY;
      //   }
      // } else if (userRole === USER_ROLE.BASIC_USER) {
      //   if (key === PermissionType.ApprovalRequests) {
      //     per[key] = PERMISSION_LEVELS.NO_ACCESS;
      //   } else if (key === PermissionType.PublicApiAccess) {
      //     per[key] = PERMISSION_LEVELS.NO_ACCESS;
      //   } else {
                  ...
                  return (
                    ...
                    >
                      <div
                        ...
                        onClick={() => (disabled ? undefined : onRoleChange(item.role))}
                      >
                        <Typography variant="body-semibold">{item.label}</Typography>
              ...
              (item: any, index: number) => (
                <TableRow key={index}>
                  {Object.keys(item).map(key => (
                    ...
                    >
                      <div className={styles.permission_info_wrapper}>
