  ...
  return (
    <Tooltip title={label.length > 30 ? label : undefined}>
      {url ? (
        ...
        <Typography color={carterColors['grey-600']} variant="caption-regular">
          {truncateString(label)}
        </Typography>
      )}
    </Tooltip>
...
const PageHeader: React.FC<PageHeaderProps> = props => {
  const { isLoading, title, onPressBackButton, backUrl, showBackButton, breadcrumbs, ActionComponent } = props;
